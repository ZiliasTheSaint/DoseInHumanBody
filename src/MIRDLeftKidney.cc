...
G4VPhysicalVolume* MIRDLeftKidney::Construct(const G4String& volumeName,G4VPhysicalVolume* mother,
						    const G4String& colourName, G4bool wireFrame, G4bool sensitivity)
{
 ...
 //infinite plane=>max coord
 G4double xx = 2.0*x1;//scaleXY*6. * cm; //2*x1=>semiAxe=x1
 G4double yy = ct;//scaleXY*12.00*cm; 
 G4double zz = ct;//scaleZ*12.00*cm;
 ...
 G4SubtractionSolid* leftKidney = new G4SubtractionSolid("LeftKidney",
						     ...
						     G4ThreeVector(//-scaleXY*6. *cm, // x1 =>shift with 2x1 but x1 semiaxes=>cut at -2x1+x1=-x1
							  -2.0*x1,//cut at the left of left kidney!!
								   0.0 *cm,
								   0.0 * cm));
  ...
  G4VPhysicalVolume* physLeftKidney = new G4PVPlacement(0 ,G4ThreeVector(x0,//scaleXY*6.*cm,  // xo
								     y0,//scaleXY*6. *cm, //yo
  ...
  VOL=LeftKidneyVol;RHO=LeftKidneyDensity;
