...
MIRDLeftScapula::MIRDLeftScapula()
{
}
...
G4VPhysicalVolume* MIRDLeftScapula::Construct(const G4String& volumeName, G4VPhysicalVolume* mother, 
						 const G4String& colourName, G4bool wireFrame,G4bool sensitivity)
{
  ...
  G4double xx = ax_out * sintheta;//0.242 ; //(sintheta;//0.242 ; //(sin 14deg) relative to outer scapula!!!!!!!!!
  G4double yy  = - ax_out * costheta;//0.97; //costheta;//0.97; // (cos 14 deg)
						   
  G4RotationMatrix* rm = new G4RotationMatrix();
  rm -> rotateZ(-theta);//-14.* degree);//- here relative to first solid!!!!!!!!!!
  //it is like 1st solid is rotated clockwise by theta and box is applied at g4threevector coordonate of this rotated solid!! ok!
 ...
 G4LogicalVolume* logicLeftScapula = new G4LogicalVolume(scapula,
  ...
  G4VPhysicalVolume* physLeftScapula = new G4PVPlacement(0,                                
				...
				G4ThreeVector(0. * cm, 0. * cm, 0.5*(0.0-ct)+zcenter),//
      			       "physicalLeftScapula",
  			       logicLeftScapula,
			       mother,
			       false,
  ...
  { 
     ...
     logicLeftScapula->SetSensitiveDetector( SDman->FindSensitiveDetector("BodyPartSD") );
   }
  ...
  VOL=LeftScapulaVol;RHO=LeftScapulaDensity;
