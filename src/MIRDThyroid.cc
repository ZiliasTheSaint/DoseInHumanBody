...
G4VPhysicalVolume* MIRDThyroid::Construct(const G4String& volumeName, G4VPhysicalVolume* mother, 
...
G4SubtractionSolid* thyroid = new G4SubtractionSolid("SecondThyroid",
 ...
 rm -> rotateZ(180.*degree);
  ...
  VOL=ThyroidVol;RHO=ThyroidDensity;
  return physThyroid;
  */
...
MirdMapConstants* mmc = new MirdMapConstants();
  mmc->initThyroid();
  std::map<std::string,thyroid_struct> esp = mmc->GetThyroidMap();    
